...
`build_android.sh` creates `.venv/` and installs host deps from `requirements-dev.txt`. It also rsyncs sibling `pydisplay/src/utils/` into `p4a_app/utils/` (full tree: `tft_config`, fonts, … — not only `path`/`mip`). Override the source with `PYDISPLAY_UTILS`. Runtime packages (**displaydev**, **pydisplay-desktop**, …) come from TestPyPI via `p4a_recipes/`. For local iteration only, pass `--local-modules` (or `ANDROID_DEBUG_LOCAL_MODULES=1`) to shadow `displaydev/`, `usdl2.py`, and Android audio modules from sibling checkouts into `p4a_app/`; default builds remove those shadows. Package id: **`org.pydevices.launcher`**. Launcher label comes from `title` in `p4a_app/buildozer.spec` (**PyDevices Launcher**).
